...
from config import default_mcp_config, default_sys_prompt, model_options
        ...
        with antd.Form.Item(label_col=24):
            with ms.Slot('extra'):
                with antd.Flex(justify='end', elem_style=dict(marginTop=10)):
                    mcp_config_confirm_btn = antd.Button(
                        '保存配置', type='primary')
            with antd.Flex(
                    wrap=True,
                ...
                with antd.Flex(gap='middle', wrap=True):
                    with antd.Flex(gap='small', align='center'):
                        antd.Typography.Text(
                            'MCP Servers', elem_style=dict(fontSize=14))
                        antd.Typography.Text(
                            '编辑以下内容以修改运行中的 MCP Servers',
                    ...
                    with antd.Button(
                            '重置默认配置', size='small') as reset_mcp_config_btn:
                        with ms.Slot('icon'):
        ...
        with antd.Form.Item(form_name='model', label='模型'):
            with ms.Slot('extra'):
                with ms.Fragment(visible=False) as thought_tip:
                    antd.Typography.Text(
                        'Note: 推理模式在调用工具前，会有较长的思考过程，需耐心等待。',
                        elem_style=dict(fontSize=12),
                        type='warning')

                with antd.Flex(
                        align='center', gap=4, elem_style=dict(marginTop=4)):
                    ms.Text('Powered by')
            ...
            with antd.Select(options=model_options) as model_select:
                with ms.Slot(
                    ...
                    with antd.Flex(gap='small'):
                        ...
                        antd.Tag(
                            elem_style=dict(display='none'), as_item='tag')
                        antd.Typography.Link(
                            '模型链接', href_target='_blank', as_item='link')
                with ms.Slot(
                        'optionRender',
                        params_mapping="""(option) =>  ({
