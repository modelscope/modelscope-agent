...
from prompt import get_action_prompt, get_memory_prompt, get_reflect_prompt, get_process_prompt, get_system_prompt
...
class MobileAgent(Agent):
    ...
    def _run(self, user_query, **kwargs):
        ...
        while True:
            ...
            # decision
            prompt_action = get_action_prompt(user_query, perception_infos, width, height, keyboard, summary_history, action_history, summary, action, add_info, error_flag, completed_requirements, memory)
            ...
            action = output_decision.split("### Action ###")[-1].split("### Operation ###")[0].replace("\n", " ").replace("  ", " ").strip()
            ...
            # reflect
            prompt_reflect = get_reflect_prompt(user_query, last_perception_infos, perception_infos, width, height, last_keyboard, keyboard, summary, action, add_info)
            ...
            self._parse_image_url([last_screenshot_file, screenshot_file], messages)
            ...
            if 'A' in reflect:
                ...
                prompt_memory = get_process_prompt(user_query, thought_history, summary_history, action_history, completed_requirements, add_info)
