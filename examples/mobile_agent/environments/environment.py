...
from environments.utils import generate, load_model, ocr, merge_text_blocks, draw_coordinates_on_image, det, crop, get_all_files_in_folder, encode_image
...
class ADBEnvironment:
    def __init__(self, adb_path:str, groundingdino_dir:List[str]) -> None:
        ...
        self.groundingdino_model = load_model(groundingdino_dir[0], groundingdino_dir[1], device="cuda").eval()
        ...
        self.screenshot_file = os.path.join(self.screenshot_dir, "screenshot.jpg")
        self.last_screenshot_file = os.path.join(self.screenshot_dir, "last_screenshot.jpg")

    def observe(self):
        perception_infos, width, height, keyboard = self.get_perception_infos(self.screenshot_file)
    ...
    def act(self, action):
        if "Open app" in action:
            ...
            text, coordinate = ocr(self.screenshot_file, self.ocr_detection, self.ocr_recognition)
            tap_coordinate = [0, 0]
            for ti in range(len(text)):
                if app_name == text[ti]:
                    name_coordinate = [int((coordinate[ti][0] + coordinate[ti][2])/2), int((coordinate[ti][1] + coordinate[ti][3])/2)]
                    self.tap(name_coordinate[0], name_coordinate[1]- int(coordinate[ti][3] - coordinate[ti][1]))# 
    ...
    def take_screenshots(self, num_screenshots, output_folder, crop_y_start, crop_y_end, slide_y_start, slide_y_end):
        adb_path = self.adb_path
