...
def test_memory_with_rag_multi_modal():
    from modelscope_agent.rag.reader.image import DashscopeParser
    memory = MemoryWithRag(
    ...
    print(summary_str)
    assert 'indexing' in summary_str or 'Indexing' in summary_str


@pytest.mark.skipif(IS_FORKED_PR, reason='only run modelscope-agent main repo')
def test_memory_with_rag_multi_modal_ms():
    from modelscope_agent.rag.reader.image import ModelscopeParser
    memory = MemoryWithRag(
        urls=['tests/samples/ms_intro.png'],
        use_knowledge_cache=False,
        image_parser=ModelscopeParser(),
    )

    summary_str = memory.run('ModelScope 社区成立于什么时候？')
    print(summary_str)
    assert '2022' in summary_str
