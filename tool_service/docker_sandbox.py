# Code borrowed from opendevin project https://github.com/OpenDevin/OpenDevin/blob/main/opendevin/sandbox/sandbox.py
import atexit
import os
import select
import sys
...
CONTAINER_IMAGE = os.getenv('SANDBOX_CONTAINER_IMAGE',
                            'modelscope-agent/tool-node:latest')
...
class DockerInteractive:
    ...
    def __init__(self,
        ...
        if workspace_dir is not None:
            assert os.path.exists(
                workspace_dir), f'Directory {workspace_dir} does not exist.'
            # expand to absolute path
            self.workspace_dir = os.path.abspath(workspace_dir)
        else:
            self.workspace_dir = os.getcwd()
            print(
                f'workspace unspecified, using current directory: {workspace_dir}'
            )
        ...
        self.execute(
            'useradd --shell /bin/bash -u {uid} -o -c \"\" -m devin && su devin'
        )
    ...
    def read_logs(self) -> str:
        ...
        logs = ''
        while True:
            ready_to_read, _, _ = select.select([self.log_generator], [], [],
...
if __name__ == '__main__':
    ...
    docker_interactive = DockerInteractive(workspace_dir=args.directory, )
    print(
        "Interactive Docker container started. Type 'exit' or use Ctrl+C to exit."
    )

    bg = DockerInteractive(workspace_dir=args.directory, )
