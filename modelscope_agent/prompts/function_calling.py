...
from modelscope_agent.prompts.react import ReAct
from modelscope_agent.prompts.react_chat import ReActChat
    ...
    def _run(self,
        ...
        if not self.llm.support_function_calling():
            if self.llm.support_raw_prompt():
                ...
                react_chat = ReActChat(
                    function_list=self.function_list, llm=self.llm)
                return react_chat.run(user_request, history=history, lang=lang)
        else:
            logger.info('beging function calling... ')
            return self._run_with_func_call(
                user_request, history=history, lang=lang)
