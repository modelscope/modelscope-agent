...
from modelscope_agent.utils.qwen_agent.schema import (ASSISTANT,
                                                      ...
                                                      Message)
from modelscope_agent.utils.qwen_agent.settings import DEFAULT_MAX_INPUT_TOKENS
from modelscope_agent.utils.qwen_agent.tokenization_qwen import tokenizer
from modelscope_agent.utils.qwen_agent.utils import (
    extract_text_from_message, format_as_multimodal_message,
    format_as_text_message, has_chinese_messages, json_dumps_compact,
    ...
    def chat(
        ...
        if self.cache is not None:
            ...
            if cache_value:
                ...
                if _return_message_type == 'message':
                    cache_value: List[Message] = [
                        Message(**m) for m in cache_value
                    ]
                if stream:
        ...
        if stream and delta_stream:
            logging.warning(
                ...
                'Using `delta_stream=True` makes it difficult to implement advanced postprocessing '
                'and retry mechanisms.')

        generate_cfg = merge_generate_cfgs(
            base_generate_cfg=self.generate_cfg,
        ...
        if 'lang' in generate_cfg:
            ...
            lang: Literal['en', 'zh'] = 'zh' if has_chinese_messages(
                messages) else 'en'
        ...
        max_input_tokens = generate_cfg.pop('max_input_tokens',
        ...
        if 'function_choice' in generate_cfg:
            ...
            valid_fn_choices = [
                f.get('name', f.get('name_for_model', None))
                for f in (functions or [])
            ]
            valid_fn_choices = ['auto', 'none'
                                ] + [f for f in valid_fn_choices if f]
...
def _postprocess_stop_words(messages: List[Message],
                            stop: List[str]) -> List[Message]:
    messages = copy.deepcopy(messages)
...
def _truncate_input_messages_roughly(messages: List[Message],
    ...
    def _truncate_message(msg: Message,
                          max_tokens: int,
                          keep_both_sides: bool = False):
        if isinstance(msg.content, str):
            content = tokenizer.truncate(
                msg.content,
            ...
            text = '\n'.join(text)
            content = tokenizer.truncate(
                text, max_token=max_tokens, keep_both_sides=keep_both_sides)
    ...
    if (sys_msg is not None
            and len(new_messages) < 2) or (sys_msg is None
                                           and len(new_messages) < 1):
        raise ModelServiceError(
            code='400',
            message=
            f'The input messages exceed the maximum context length ({max_tokens} tokens) after '
...
def retry_model_service(
    ...
    while True:
        try:
            ...
            num_retries, delay = _raise_or_delay(e, num_retries, delay,
                                                 max_retries)
...
def _raise_or_delay(
    ...
    logging.warning('ModelServiceError - ' + str(e).strip('\n'))
