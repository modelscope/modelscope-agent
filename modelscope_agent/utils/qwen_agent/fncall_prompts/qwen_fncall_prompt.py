...
import json
from modelscope_agent.utils.qwen_agent.fncall_prompts.base_fncall_prompt import \
    BaseFnCallPrompt
from modelscope_agent.utils.qwen_agent.schema import (ASSISTANT, FUNCTION,
                                                      SYSTEM, USER,
                                                      ContentItem,
                                                      FunctionCall, Message)
from modelscope_agent.utils.qwen_agent.utils import extract_text_from_message
...
class QwenFnCallPrompt(BaseFnCallPrompt):
    ...
    def preprocess_fncall_messages(messages: List[Message],
        ...
        for msg in copy.deepcopy(ori_messages):
            ...
            if role in (SYSTEM, USER):
                ...
                if last_text_content.endswith(f_exit):
                    messages[-1].content[
                        -1].text = last_text_content[:-len(f_exit)]
                f_result = [ContentItem(text=f'\n{FN_RESULT}: ')
                            ] + f_result + [ContentItem(text=f_exit)]
        ...
        tool_descs = '\n\n'.join(
            get_function_description(function, lang=lang)
        ...
        if function_choice not in ('auto', 'none'):
            if messages[-1].role == ASSISTANT:
                ...
                if last_msg.content:
                    if extract_text_from_message(
                            last_msg, add_upload_info=False).endswith(FN_EXIT):
            ...
            last_msg.content.append(
                ContentItem(text=f'{FN_NAME}: {function_choice}'))
    ...
    def postprocess_fncall_messages(messages: List[Message],
        ...
        for msg in messages:
            ...
            if role in (SYSTEM, USER):
                new_messages.append(
                    Message(role=role, content=content, extra=extra))
                continue
            ...
            for item in content:
                ...
                for part in item_text.split(f'{FN_NAME}:'):
                    ...
                    if i < 0:
                        ...
                        list_of_fn_args = [
                            _.strip()
                            for _ in part[i + len(arg_sep):].split(arg_sep)
                        ]
...
please insert the following command in your reply, which can be called zero or multiple times according to your needs:
