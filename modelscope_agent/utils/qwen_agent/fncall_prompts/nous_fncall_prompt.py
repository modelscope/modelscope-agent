    ...
    def preprocess_fncall_messages(self,
        ...
        for msg in copy.deepcopy(ori_messages):
            ...
            if role in (SYSTEM, USER):
                ...
                if fn_call:
                    if (not SPECIAL_CODE_MODE) or (CODE_TOOL_PATTERN
                                                   not in fn_call.name):
                        fc = {
                            'name': fn_call.name,
                ...
                if messages and messages[-1].role == ASSISTANT:
                    if messages[-1].content and messages[-1].content[
                            -1].text and (not messages[-1].content[-1].text.
                                          endswith('\n')):
        ...
        if (SPECIAL_CODE_MODE
                and any([CODE_TOOL_PATTERN in x for x in tool_names])):
            tool_system = FN_CALL_TEMPLATE_WITH_CI.format(
        ...
        for msg in messages:
            ...
            for item in content:
                ...
                if thought_in_content:
                    ...
                    new_content.append(
                        ContentItem(text='</think>'.join(_item_text[:-1])
                                    + '</think>'))
                ...
                for txt in tool_call_list[1:]:
                    ...
                    if new_content:
                        ...
                        new_content = []
                    if SPECIAL_CODE_MODE and '<code>' in one_tool_call_txt[
                            0] and '</code>' in one_tool_call_txt[0]:
