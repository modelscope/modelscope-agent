...
from .action_parser import MsActionParser, MRKLActionParser, OpenAiFunctionsActionParser
from modelscope_agent.constant import DEFAULT_MODEL_CONFIG


class ActionParserFactory:
    @classmethod
    def get_action_parser(cls, agent_type: AgentType = AgentType.DEFAULT, model: LLM = None, cfg: Dict = None, **kwargs):
    ...
    def _string_to_obj(cls, action_parser_name: str, **kwargs):
        print(f'action_parser_register.registered: {action_parser_register.registered}')
        for parser in action_parser_register.registered:
            print(f'action_parser_name: {action_parser_name}, parser.__name__: {parser.__name__}')
            if action_parser_name == parser.__name__:
                obj = parser(**kwargs)
                return obj
        raise ValueError(f'prompt parser {action_parser_name} is not registered.')
    ...
    def _get_action_parser_by_agent_type(cls, agent_type: AgentType = AgentType.DEFAULT):
