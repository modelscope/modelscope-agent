...
from modelscope_agent.agents.codexgraph_agent.utils.prompt_utils import load_prompt_template, build_system_prompt, response_to_msg
from modelscope_agent.agents.codexgraph_agent.prompt import JSON_PROMPT
from modelscope_agent.agents.codexgraph_agent.utils.code_utils import extract_and_parse_json
...
class CodexGraphAgentGeneral(Agent, ABC):
    def __init__(self,
        ...
        system_prompts, cypher_system_prompts = build_system_prompt(prompt_path, schema_path, language=language)

        self.system_prompts = system_prompts
        self.primary_user_prompt_template = load_prompt_template(prompt_path, 'start_prompt_primary.txt', language=language)
        self.generate_queries_template = load_prompt_template(prompt_path, 'generate_prompt.txt', language=language)
        self.cypher_queries_template = load_prompt_template(prompt_path, 'start_prompt_cypher.txt', language=language)

        self.cypher_agent = CypherAgent(llm, graph_db=graph_db, system_prompts=cypher_system_prompts, task_id=task_id)
    ...
    def _run(self,
        ...
        for iter in range(self.max_iterations):
            ...
            if error_msg:
                user_response = f"Something wrong with the JSON format, please rewrite it and follow the given format:\n{JSON_PROMPT}"
                messages.append({"role": "user", "content": user_response})
                continue

            thought, action, action_input = parsed_response.values()

            self.update_agent_message(response_to_msg(thought, action, action_input))
            ...
            if action == self.action_type:
                ...
                user_response = f"Invalid action, the action should be `{self.action_type}` or `TEXT_QUERIES`"
