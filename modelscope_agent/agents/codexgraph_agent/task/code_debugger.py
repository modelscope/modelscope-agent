...
from modelscope_agent.agents.codexgraph_agent.utils.prompt_utils import load_prompt_template, replace_system_prompt
...
def response_to_msg(extraced_analysis, extraced_code_search, extraced_bug_location):
    msg = ""
...
class CodexGraphAgentDebugger(CodexGraphAgentGeneral):
    def __init__(self,
        ...
        self.cypher_queries_buggy_template = load_prompt_template(prompt_path, 'start_prompt_cypher_buggy_loc.txt')
    ...
    def _run(self,
        ...
        for iter in range(self.max_iterations):
            ...
            extracted_bug_location, _ = extract_text_between_markers(response_text, "[start_of_bug_locations]", "[end_of_bug_locations]")

            self.update_agent_message(response_to_msg(extracted_analysis, extracted_code_search, extracted_bug_location))

            if not extracted_code_search and not extracted_bug_location:
                msg = "The text between the markers [start_of_code_search] and [end_of_code_search], as well as the text between the markers [start_of_bug_locations] and [end_of_bug_locations], is empty."
                ...
                cypher_queries = self.cypher_queries_buggy_template.substitute(text_queries=extracted_code_search)
                user_response = self.cypher_agent.run(cypher_queries, retries=self.max_iterations_cypher)
                ...
                if "Node" not in user_response:
                    collated_tool_response += f"\n\nIt seems that buggy locations are missing. Please try again."
                    messages.append({"role": "user", "content": collated_tool_response})
                    continue
            ...
            if iter < self.max_iterations - 1:
                msg = (
                    ...
                    "\n- If you haven't yet reviewed the specific code related to the bug or pinpointed the exact location of the bug (such as which module, class, method, or function), it is not recommended to provide answers that directly specify the bug locations."
                )
                messages.append({"role": "user", "content": msg})
