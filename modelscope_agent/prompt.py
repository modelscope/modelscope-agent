...
class MSPromptGenerator(PromptGenerator):
    ...
    def init_prompt(self, task, tool_list, available_tool_list):
        ...
        # TODO: tmp search logic
        re_search_pattern = re.compile(
            r'<\|startofsearch\|>([\s\S]+)<\|endofsearch\|>([\s\S]+)')

        try:
            pattern_content = re_search_pattern.search(task)
            search_content, task_content = pattern_content.group(
                1), pattern_content.group(2)

            task = f'{search_content}\n{task_content}'
            task_remove_search = task_content
        except Exception:
            task_remove_search = task
        ...
        tool_list_str = '\n\n'.join([
            f'{i+1}. {str(tool_list[t])}'
            for i, t in enumerate(available_tool_list)
        ])
        ...
        # store history
        self.history.append({'role': 'user', 'content': task_remove_search})
