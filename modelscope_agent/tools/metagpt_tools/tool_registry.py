...
class ToolRegistry(BaseModel):
    ...
    tools_by_tags: dict = defaultdict(
        dict)  # two-layer k-v, {tag: {tool_name: {...}, ...}, ...}
    ...
    def register_tool(
        ...
        if not schemas:
            schemas = make_schema(tool_source_object, include_functions,
                                  schema_path)
        ...
        schemas[
            'tool_path'] = tool_path  # corresponding code file path of the tool
        try:
        ...
        if verbose:
            ...
            print(
                f'schema made at {str(schema_path)}, can be used for checking')

    def has_tool(self, key) -> Tool:
...
def register_tool(tags: list[str] = None, schema_path: str = '', **kwargs):
    ...
    def decorator(cls):
        ...
        if 'modelscope_agent' in file_path:
            # split to handle ../metagpt/metagpt/tools/... where only metapgt/tools/... is needed
            file_path = './modelscope_agent' + file_path.split(
                'modelscope_agent')[-1]
...
def make_schema(tool_source_object, include, path):
    os.makedirs(
        ...
        exist_ok=True)  # Create the necessary directories
    try:
        schema = convert_code_to_tool_schema(
            tool_source_object, include=include)
        with open(path, 'w', encoding='utf-8') as f:
            yaml.dump(schema, f, sort_keys=False)
    except Exception as e:
        schema = {}
        print(f'Fail to make schema: {e}')
...
def validate_tool_names(tools: list[str]) -> dict[str, Tool]:
    ...
    for key in tools:
        ...
        if os.path.isdir(key) or os.path.isfile(key):
            ...
            logger.warning(
                f'invalid tool name or tool type name: {key}, skipped')
    return valid_tools
...
def register_tools_from_file(file_path) -> dict[str, Tool]:
    ...
    if not file_name.endswith(
            '.py') or file_name == 'setup.py' or file_name.startswith('test'):
        return {}
    registered_tools = {}
